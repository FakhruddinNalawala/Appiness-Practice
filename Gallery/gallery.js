    ...
    .then(function(querySnapshot) {
        ...
        for(var i=0;i<nameList.length;i++)  {
            if (!finalLinks.includes(nameList[i]))  {
                ...
                // const image = `
                //     <div class='four wide center aligned column button-holder' id='`+idfy(nameList[i])+`' style="background-image:url('`+fileList[i]+`');">
                ...
                //     </div>
                // `;
                // document.getElementById("directory").innerHTML += image;
                // console.log('running');

                var listRef = firebase.storage().ref().child('root/images');
                var images = [];

                // Find all the prefixes and items.
                listRef.listAll().then(function(res) {
                    res.items.forEach(function(itemRef) {
                        firebase.storage().refFromURL(String(itemRef)).getDownloadURL().then(function(url) {
                            console.log(url);
                            if (!images.includes(url))   {
                                document.getElementById('directory').innerHTML += `<div id = '`+url+`' class = 'eleven wide center aligned column button-holder'><img src=`+url+` width='60%'></img></div>`;
                            }
                            images.push(url);
                        });
                ...
                // var xxx = idfy(nameList[i]);
                // firebase.storage().refFromURL(fileList[i]).getDownloadURL().then(function(url) {
                //     console.log(url, xxx);
                //     document.getElementById(xxx).style.backgroundImage = "url("+url+") !important"
                // });
                // finalLinks.push(nameList[i]);
            }
        }
